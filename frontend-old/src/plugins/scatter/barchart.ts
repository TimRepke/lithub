...
export default (container: d3.Selection<HTMLDivElement, never, HTMLDivElement, never>,
                ...
                width: number, height: number, color: (x: number) => d3.RGBColor,skip:number|undefined, margin: Margins | undefined) => {
  ...
  barchart.update = (counts: Map<string, number> | Map<number, number>) => {
    //@ts-ignore
    bars.data(counts.entries())
    bars
      .attr("height", d => height - y(d[1]))
      .attr("y", d => y(d[1]));
  }
  barchart.update(counts);
