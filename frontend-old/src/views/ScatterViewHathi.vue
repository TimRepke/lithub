...
import {seriesSvgAnnotation} from '@/plugins/hathi/annotationSeries.js';

import * as d3 from 'd3';
import * as fc from 'd3fc';
import * as Arrow from 'apache-arrow';
import bespokePointSeries from '@/plugins/hathi/bespokePointSeries';
import streamingAttribute from '@/plugins/hathi/streamingAttribute';
import indexedFillColor from '@/plugins/hathi/indexedFillColor';
import closestPoint from '@/plugins/hathi/closestPoint';
...
const columnValues = (table, columnName) => {
  const index = data.columns[columnName];
  return table.batches
      .filter(chunk => chunk.numRows > 0 && chunk.numCols > 0)
      .map((chunk) => chunk.data.children[index].values);
};
...
const xScale = d3.scaleLinear().domain([-55, 55]);
const yScale = d3.scaleLinear().domain([-55, 55]);
...
const createAnnotationData = (row) => {
  return {
    ix: row['idx'],
    note: {
      label: `label published in ${row['publication_year']}`,
      bgPadding: 5,
      title: `title for ${row['idx']}`,
    },
    data: {
      x: row['x'],
      y: row['y'],
...
function redraw() {
  ...
  if (!LO_FI_VERSION) {
    ...
    yearAttribute.data(columnValues(data.table, 'publication_year'));
    indexAttribute.data(columnValues(data.table, 'idx'));
  }
...
const loadData = async () => {
  ...
  const response = await fetch('http://127.0.0.1:8082/raw/carbonprice.min.feather');
  const reader = await Arrow.RecordBatchReader.from(response);
  await reader.open();
  data.table = new Arrow.Table(reader.schema);
  data.columns = Object.fromEntries(data.table.schema.names.map((name: string, i: number) => [name, i]))
  for await (const recordBatch of reader) {
    // console.log(data.table)
    data.table = data.table.concat(recordBatch);
    // console.log(data.table)
    document.querySelector('#loading>span').innerHTML =
        new Intl.NumberFormat().format(data.table.numRows) + ' points loaded';
    redraw();
  }
};
...
#chart d3fc-group {
  grid-template-columns: 1px auto 1fr 4px 0;
  grid-template-rows: 1px auto 1fr 4px 0;
}
