...
OUT_FULL = Path('data/carbonprice.full.feather')
...
engine = create_engine('sqlite:///data/carbonprice.slim.sqlite', echo=False)
with engine.begin() as connection:
    df_slim.to_sql(name='data', con=connection, if_exists='append',
                   dtype={
                       **{
                           f'{key}|{int(value)}': Boolean()
                           for key, label in LABELS.items()
                           for value in label['values'].keys()
                       },
...
engine = create_engine('sqlite:///data/carbonprice.full.sqlite', echo=False)
with engine.begin() as connection:
    df_full.to_sql(name='data', con=connection, if_exists='append')
